...
| `KSTART` / `KEND` / `KSTEP` | K sweep range. In `MODE=BISECT`/`BISECT_SECANT`, `KSTEP` means stop-tolerance (bisection halts once the bracket narrows to this width) instead of a fixed grid step. | 0.0 / 0.08 / 0.005 |
