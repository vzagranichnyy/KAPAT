...
# 5. Определение папки с веб-сервером
if [ -f "$KAPAT_DIR/prusa_pa_tuner/app.py" ]; then
    APP_DIR="$KAPAT_DIR/prusa_pa_tuner"
elif [ -f "$KAPAT_DIR/app.py" ]; then
    APP_DIR="$KAPAT_DIR"
else
    APP_DIR="$KAPAT_DIR"
fi

# 6. Создание папки для логов (чтобы сервер не упал при первом сохранении)
...
ExecStart=$KAPAT_DIR/venv/bin/uvicorn app:app --host 0.0.0.0 --port 8000
Restart=always
RestartSec=10
...
echo ""
echo "❗ ВАЖНО: Не забудьте добавить [include kapat.cfg] в ваш printer.cfg!"
echo ""
